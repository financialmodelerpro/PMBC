...
16. **Three collection pages are linked but empty.** `/team`, `/case-studies` and `/insights` are in the footer and the sitemap and now open with a proper 630px hero, but all three collections have no rows, so each is a hero over an empty state. They were never in the navbar, which is why they were kept when the footer was trimmed, but a linked empty page is a weaker impression than no page. Either populate them or drop them from the footer and sitemap. [user decides]
...
| `/fmp` | financial-modeler-pro | The platform arm in full: hero with capability tags, what FMP is, who it is for (an `audience_carousel` since migration 053), the Modeling and Training Hubs, the two certification paths, CTA. **Moved from `/financial-modeler-pro`, which 301s here** (migration 049). The three sub-pages beneath the old path are retained, unlinked and out of the sitemap. |
