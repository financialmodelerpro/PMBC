...
All buildable features are done as of 2026-06-01. Page content is seeded, the public site renders, and the admin console (CMS, page builder, contact inbox) is complete. What is left is operational and review work, most of which lives outside the codebase. Pick up here next session.
...
2. **Rotate `Admin@2026`** to a strong production credential via `npm run seed-admin`. [assistant can set the password in `scripts/seed-admin.mjs`; user runs the script]
