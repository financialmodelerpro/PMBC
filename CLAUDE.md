...
- **Growth Engine: suppression always wins.** Every send calls `checkSuppression` (`src/lib/growth/suppression.ts`) first and sends only when it returns not suppressed; it fails closed. Send limits, the AI budget and retention are read through `getGrowthSettings` (`src/lib/growth/settings.ts`), never hardcoded, and nothing sends or spends when the row cannot be read or the budget is not set. **Every Growth AI call goes through `runAi`** (`src/lib/growth/ai/run.ts`): it checks the Knowledge Base, the budget and Riyadh-month spend, records the call and logs it with actor `ai`. No agent imports a provider or the Anthropic SDK directly. Without `ANTHROPIC_API_KEY` the layer answers from the labelled mock; mock output must stay labelled wherever it is shown. **Growth services are the site's nine services**, read from `src/config/services.ts` through `GROWTH_SERVICES`; never keep a separate service list.
- **Growth public pieces are built off.** The website chat reaches the public site only through `ChatWidgetMount` in the public layout, which renders nothing unless `chat_widget_enabled` is on (default off) and `ANTHROPIC_API_KEY` is set; the widget is a plain script (`/api/growth/widget`), never a React component in the layout, which would ship to every page. Tracked-link and opt-out routes live under `/api/growth/*`.
...
`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `NEXT_PUBLIC_SITE_URL`, `BREVO_API_KEY`, `EMAIL_FROM_DEFAULT`, `EMAIL_FROM_NAME`, `EMAIL_TO_ADMIN`, `EMAIL_FROM_CONTACT` (optional, the From on the acknowledgement), `FMP_API_URL`, `FMP_API_KEY` (server only; FMP fails closed, and PMBC then serves its stored copy), `HCAPTCHA_SECRET_KEY` and `NEXT_PUBLIC_HCAPTCHA_SITE_KEY` (not required; setting both re-enables hCaptcha), `TOOL_LEAD_IP_SALT` (IP hashing, falls back to `NEXTAUTH_SECRET`), `ANTHROPIC_API_KEY` (optional; unset, the Growth AI layer runs in mock mode), `BREVO_WEBHOOK_TOKEN` (unset, the webhook answers 503), `CRON_SECRET` (unset, the reminder and Growth crons answer 503), `NEXT_PUBLIC_GA_ID` (optional). Growth Engine, all optional (each missing one keeps its part in labelled mock mode): `MS_GRAPH_TENANT_ID`, `MS_GRAPH_CLIENT_ID`, `MS_GRAPH_CLIENT_SECRET`, `MS_GRAPH_SENDER` (outreach mail and replies), `MS_BOOKINGS_BUSINESS_ID` (Bookings sync), `GROWTH_BREVO_LIST_ID` (nurture; the site's Brevo key alone never sends marketing mail), `GROWTH_BREVO_WEBHOOK_TOKEN` (unset, the Growth Brevo events route answers 503).
