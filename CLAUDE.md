...
16. **Three collection pages are empty, and are now unlinked from the footer.** `/team`, `/case-studies` and `/insights` have no rows, so each is a hero over an empty state. Their footer links were seeded hidden on 2026-08-13 rather than deleted, so each is one switch in **Footer Links** away from returning. **They are still in the sitemap**, which is the remaining half of the decision: a page nothing links to but the sitemap advertises is the same half-state `/approach` is in. Either populate them, or take the three lines out of `sitemap.ts` as well. [user decides]
...
- **Legal**, in the bottom strip: Privacy, Terms. Not editable in Footer Links, by design.
