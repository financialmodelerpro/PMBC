...
**Working admin login (local dev):** `meetahmadch@gmail.com` / `Admin@2026`. This is a debug-only password, and it is in public git history, so treat it as compromised: it must be rotated to a strong production credential before launch.
...
2. **Rotate `Admin@2026`** to a strong production credential via `npm run rotate-admin-password`. **[user runs the script, and only the user: the tooling is deliberately built so the plaintext never passes through the assistant, the repo, or a chat transcript. Do not paste the new password into a chat.]** The script self-verifies; afterwards the assistant can confirm rotation without knowing the password by checking that the stored hash changed and that the old password is refused.
