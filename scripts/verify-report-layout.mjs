...
import { REPORT_META, VALUATION_DATE } from './lib/valuationCases.mjs';
import { makeLayoutCases } from './lib/layoutCases.mjs';
...
const { build, combos: allCombos } = makeLayoutCases(state);
...
async function pages(buf) {
  ...
  for (let n = 1; n <= doc.numPages; n++) {
    ...
    out.push({ text: items.map((i) => i.str).join(' ').replace(/\s+/g, ' '), lowest: Math.max(0, ...body.map((i) => H - i.transform[5])), marks: items.filter((i) => i.str === 'PASS' || i.str === 'WARNING').length });
  }
  return out;
}

console.log(`Report layout: ${combos.length} combinations`);
let worst6 = Infinity, worstCase = null, refused = 0, together = 0, rendered = 0;
for (const [k, c] of combos.entries()) {
  ...
  // The check list: kept on one page whenever the estimate says it fits, and that promise holds.
  const checkList = format.checkItems(r);
  const promised = pdfModule.checksFitOnPage6(r, checkList);
  const onOnePage = p.some((pg) => pg.marks === checkList.length);
  if (promised) check(`${tag}: check rows kept together on one page, as estimated`, onOnePage, p.map((pg) => pg.marks).join(','));
  rendered++;
  if (onOnePage) together++;
...
console.log(`  check rows on one page in ${together} of ${rendered} reports`);
