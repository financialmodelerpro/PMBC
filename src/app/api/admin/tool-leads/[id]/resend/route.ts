 ...
 * results stored on the lead. Never recomputed: the visitor receives the same
...
export async function POST(_req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const sent = await sendResultsEmail(lead, reviveResult(lead.results), { resend: true, adminId: session.user.id });
