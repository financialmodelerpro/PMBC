...
Tell the build session "088 applied" (or whichever) and it re-runs the live checks that were pending. **As at 2026-09-23, 083 to 093 are applied; 094 is pending.**
...
| 094 | `094_growth_signal_keywords.sql` | Signal keyword library: growth_keyword_groups and growth_signal_keywords (one keyword per group ignoring case), growth_signals.matched_keyword_id and matched_keyword, and the feed paused on the settings row. The defaults are not seeded here: the code holds them and saves them on the first change. Safe before or after the deploy; before it, the library shows read only and the feed uses the old keyword list, which is empty | **Pending** |
