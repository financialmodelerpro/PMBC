...
| `/admin/growth` | Growth Engine (Unit 1.1, 2026-09-22): an empty section shell with its own sub-navigation over eleven pages (Home, Signals, Prospects, Outreach, Pipeline, Conversations, Meetings, Partners, Knowledge Base, Analytics, Settings), each stating its purpose and the build phase that delivers it. Pages are listed once in `src/lib/growth/pages.ts`; each page calls `requireGrowthSession` (`src/lib/growth/access.ts`) before rendering, on top of the middleware and admin layout. **Admin only since Unit 1.2**: `/admin/growth` is in `ADMIN_ONLY_PREFIXES`, the sidebar item carries `role: 'admin'`, and `requireGrowthSession` sends an editor to `/admin?denied=1`. Home shows whether the data layer (migration 083, `src/lib/growth/db.ts`) is ready, table by table. Value lists and row types are in `src/lib/growth/model.ts`; `npm run verify-growth-data` proves them against the migration. **Knowledge Base** (Unit 1.3, `/admin/growth/knowledge-base`, migration 084): ten kinds defined in `src/lib/growth/kbModel.ts`, data access in `src/lib/growth/kb.ts`. Each item has a working copy and an approved copy; editing an approved item leaves the approved copy in place until re-approved, and **agents read only `getApprovedKnowledge()`**, which returns approved copies of approved, non-test items grouped by kind. Services and offers are fixed to their ten starter items. Creation, approval, edits of approved items, archive and restore are logged to `growth_activity` by a database trigger, with the admin as actor. Order any timeline by `(created_at, seq)`. `npm run verify-growth-kb` proves it. **Settings** (Unit 1.4, `/admin/growth/settings`, migration 085): outreach limits, AI budget (null until set) and retention in one row read by `getGrowthSettings`; the suppression list and `checkSuppression`, which also reads the valuation tool unsubscribes and opted-out contacts live and fails closed; a read-only retention preview (`retention.ts`); integration status (`integrations.ts`, names only, `built` flags per integration); and the audit log view over growth_activity (`audit.ts`). `npm run verify-growth-settings` proves it. **Services** (Unit 1.3b, migration 086): the Growth services are the site's nine, read from `src/config/services.ts`; each Knowledge Base service is keyed by its site slug and links to its own page (enforced by the database), offers link to related services, and the outreach priority list is a setting. **AI layer** (Unit 1.5, migration 087): `runAi` in `src/lib/growth/ai/run.ts` is the only way to call AI. Providers sit behind `AiProvider` (`provider.ts`): `mock.ts` when `ANTHROPIC_API_KEY` is unset, `anthropic.ts` (official SDK, `claude-opus-5` by default, server-side refusal fallbacks) when it is set. Prices in `pricing.ts`; spend by Riyadh month; the budget alert is claimed once per month by a unique row. Verifiers use the isolated settings row 2. `npm run verify-growth-ai` proves it. **Phases 2 to 7** (from 2026-09-23; default model now `claude-sonnet-5`, per agent in Settings): each unit's files, rules and verifiers are in `docs/GROWTH_ENGINE_STATUS.md`; migrations in `docs/PENDING_MIGRATIONS.md` |
