...
089  growth_outreach         **DDL, HAND-RUN.** Phase 3: growth_messages (approval, scheduling, sends, replies; mock drafts never sent for real by constraint; status changes logged by trigger), growth_tracked_links and growth_link_clicks (site paths only), growth_opportunities (lost needs a reason), growth_tasks; lead sequence, reply and meeting-request columns; settings outreach_sending_paused and lead_scoring_weights (sum 100, checked). Applied 2026-09-23.
090  growth_website_chat     **DDL, HAND-RUN.** Phase 4: growth_conversations (details only with consent; consent wording and time recorded; opt-in needs consent) and growth_chat_messages; settings chat_widget_enabled (default false), limits, consent wording, lead_alert_email; unique Growth lead per valuation tool lead. Applied 2026-09-23.
091  growth_meetings         **DDL, HAND-RUN.** Phase 5: growth_meetings (unique Bookings appointment id, outcome only when held, mock never a Bookings row, brief with mock flag, notes and outcome), growth_messages.meeting_id, settings bookings_url. Applied 2026-09-23.
092  growth_nurture_partners  **DDL, HAND-RUN.** Phase 6: contact nurture columns (subscription needs opted-in consent), growth_nurture_steps, growth_lead_magnets, message opened_at and clicked_at, growth_brevo_events (unique per event), growth_partners, growth_partner_checkins, growth_introductions, lead referral columns, settings nurture_enabled (default false) and partner_checkin_days. Applied 2026-09-23.
093  growth_intelligence     **DDL, HAND-RUN.** Phase 7: growth_scoring_reviews (suggested weights sum to 100, rejection needs a note). Applied 2026-09-23.
094  growth_signal_keywords  **DDL, HAND-RUN.** Signal keyword library: growth_keyword_groups, growth_signal_keywords, growth_signals.matched_keyword_id and matched_keyword; pauses the feed. Defaults live in code (src/lib/growth/keywordLibrary.ts) and are saved on the first change. Later Growth migrations: see docs/PENDING_MIGRATIONS.md
```
